/**
 * @file conv.cpp
 * @author Liyang Xia (xialiyang2004@gmail.com)
 * @brief   一个用于处理矩阵的函数库，具体使用方法请参考函数前的注释以及测试函数main中的内容，
 * 如果需要加入自己的工程，请自行添加头文件并将函数定义加入源文件中
 * @version 0.1
 * @date 2025-05-02
 *
 * @copyright Copyright (c) 2025 XLY All power reserved
 *
 */
...
void matrix_conv(int **input, matrix *input_inx, int **kernel, int kernel_size,
                 int padding, int stride, int **output, bool is_init);
...
int main()
{
    ...
    matrix img_indx;
    int **img = read_y(fp, &img_indx);
    // printf("row is %d,col is %d\n", img_indx.row, img_indx.col);
    // for (int i = 0; i < img_indx.row; i++)
    // {
    //     for (int j = 0; j < img_indx.col; j++)
    //     {
    //         printf("%d ", img[i][j]);
    //     }
    //     printf("\n");
    ...
    int kernel[3][3] = {
        ...
        {2, 4, 2},
        {1, 2, 1}};
    int **kernel_ptr = (int **)malloc(kernel_size * sizeof(int *));
    for (int i = 0; i < kernel_size; i++)
    {
        kernel_ptr[i] = (int *)malloc(kernel_size * sizeof(int));
        for (int j = 0; j < kernel_size; j++)
        {
            kernel_ptr[i][j] = kernel[i][j];
            printf("%d ", kernel_ptr[i][j]);
        }
        printf("\n");
    }
    int pad = 2; // 填充大小
    int str = 1; // 步长
    matrix *output_size = caculate_output_size(&img_indx, kernel_size, pad, str);
    int **output_conv = (int **)malloc(output_size->row * sizeof(int *));
    for (int i = 0; i < output_size->row; i++)
    {
        output_conv[i] = (int *)malloc(output_size->col * sizeof(int));
        for (int j = 0; j < output_size->col; j++)
        {
            output_conv[i][j] = 0;
        }
    }
    matrix_conv(img, &img_indx, kernel_ptr, kernel_size, pad, str, output_conv, true);
    // printf("Convolution Result:\n");
    // for (int i = 0; i < output_size->row; i++)
    // {
    //     for (int j = 0; j < output_size->col; j++)
    //     {
    //         printf("%d ", output_conv[i][j]);
    //     }
    //     printf("\n");
    // }
    // 将卷积结果写入文件
    ...
    for (int i = 0; i < output_size->row; i++)
    {
        for (int j = 0; j < output_size->col; j++)
        {
            fprintf(out, "%d ", output_conv[i][j]);
        }
        fprintf(out, "\n");
    }
    // 将卷积之后的图像线性插值回原来的大小
    matrix output_indx;
    output_indx.row = img_indx.row;
    output_indx.col = img_indx.col;
    int **output = (int **)malloc(output_indx.row * sizeof(int *));
    for (int i = 0; i < output_indx.row; i++)
    {
        output[i] = (int *)malloc(output_indx.col * sizeof(int));
        for (int j = 0; j < output_indx.col; j++)
        {
            output[i][j] = 0;
        }
    }
    Bilinear(output_conv, output_size, output, &output_indx);
    // printf("Bilinear Interpolation Result:\n");
    // for (int i = 0; i < output_indx.row; i++)
    // {
    //     for (int j = 0; j < output_indx.col; j++)
    //     {
    ...
    // }
    // 将线性插值结果写入文件
    FILE *out2 = fopen("output_interpolation.txt", "w");
    if (out2 == NULL)
    {
        printf("打开输出文件失败\n");
        fclose(fp);
    ...
    for (int i = 0; i < output_indx.row; i++)
    {
        for (int j = 0; j < output_indx.col; j++)
        {
            fprintf(out2, "%d ", output[i][j]);
        }
        fprintf(out2, "\n");
    }
    float PSNR = caculator_PSNR(img, output, &img_indx);
    printf("%f", PSNR);
    fclose(out2); // 关闭输出文件
    fclose(out);  // 关闭输出文件
    fclose(fp);   // 记得关闭文件
